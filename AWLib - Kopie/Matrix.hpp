	...
	namespace Mathematik {
		...
		public:
			NUMERISCH operator[]( const Punkt2DU pos ) const { 
				if( pos.x() >= DimX ) throw Vektor<DimX,NUMERISCH>::Fehler( "ungueltige X Dimenson.", pos.x() );
				if( pos.y() >= DimY ) throw Vektor<DimY,NUMERISCH>::Fehler( "ungueltige Y Dimenson.", pos.y() );
		...
		public:
			static void Test( ) {
...

				{
					...
					std::cout << "mat1 - dimensonen x: = " << (int) mat1.dimensionX() << " y: " << (int) mat1.dimensionY() << std::endl;
					std::cout << "mat2 - dimensonen x: = " << (int) mat2.dimensionX() << " y: " << (int) mat2.dimensionY() << std::endl;
					...
					std::cout << "mat1: = " << mat1 << std::endl;
					std::cout << "mat2: = " << mat2 << std::endl;
					std::cout << "*     = " << mulMat << std::endl;

					std::cout << "2*mat1: = " << 2*mat1 << std::endl;
					std::cout << "mat1*2: = " << mat1*2 << std::endl;

					assert( 2*mat1 == mat1*2 );

					mat1 *= 2;
					std::cout << "mat1: *= 2" << mat1 << std::endl;
				}
			...
			for( uint8_t y = 0; y < DimY; y++ ) {
				for( uint8_t x = 0; x < DimY; x++ ) {
					...
					for( uint8_t n = 0; n < DimX; n++ ) {
						//std::cout << " "<< _data[y][n] << " * " << matrix._data[n][x] << " ";
