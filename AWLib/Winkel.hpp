	...
	namespace Mathematik {
		...
		public:
				...
				_grad(std::numeric_limits<double>::infinity())
			{
				...
				//Objekt::testAusgabe( ) << "sin: " << _sinus << " cos: " << _kosinus << std::endl;
				//Objekt::testAusgabe( ) << "asin: " << asin( _sinus ) << " acos: " << acos( _kosinus ) << std::endl;


				if( _sinus >= 0 ) { 
					if( _kosinus >= 0 ) {	// Quadrant 1
						_bogenmass = asin( _sinus );
						//Objekt::testAusgabe( ) << "Q1" << std::endl;
					} else {				// Quadrant 2
						_bogenmass = acos( _kosinus );
						//Objekt::testAusgabe( ) << "Q2" << std::endl;
					}
				} else {
					if( _kosinus < 0 ) {	// Quadrant 3
						_bogenmass = PI - asin( _sinus );
						//Objekt::testAusgabe( ) << "Q3" << std::endl;
					} else {				// Quadrant 4
						_bogenmass = 2.0*PI + asin( _sinus );
						//Objekt::testAusgabe( ) << "Q4" << std::endl;
					}
				}
